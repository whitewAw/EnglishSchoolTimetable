...
namespace BroadwayEnglishSchoolTimetable.Controllers
{
    ...
    public class calendarForTeachersOnlyController : Controller
    {
        ...
        public ActionResult Index() => View(Mapper.Map<forCalendarBL, forCalendarPL>(BLService_.forCalendarBL.Get(IdTeacher)));
        ...
        public JsonResult GetResources(string pointStar, string pointEnd)
        {
            ...
            if (numDay == 0)
                numDay = 7;
            var el = Json(Mapper.Map<IEnumerable<teacherBL>, List<teacherPL>>(BLService_.teacherBL.TakeBetweenDates(startdate, enddate).Where(p => p.Id == IdTeacher)).Select(p => new {
                Id = p.Id,
        ...
        public JsonResult GetEvents(string pointStar, string pointEnd)
        {
            ...
            List<timetableByDatePL> el = Mapper.Map<IEnumerable<timetableByDateBL>, List<timetableByDatePL>>(BLService_.timetableByDateBL.TakeBetweenDates(startdate, enddate).Where(p=>p.idTeacher==IdTeacher));
